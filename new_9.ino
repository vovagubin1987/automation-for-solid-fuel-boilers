...
signed short tmp2=tBreakOffIn-tIn;//для удобства текущая дельта между допустимой внутренней и реальной
...
if (tBreakOffIn > tIn and rew == 1){//если температура внутри менее допустимой и режим первый
  tmp1=((1023/tDeltaIn)*tmp2*kof)/1;//максимальный диапазон икм делим на дельту заданную, затем * на tmp2(не более дельты) и умножаем на коэффициент зависищий от забортной температуры для нелинейности регулировки
};
